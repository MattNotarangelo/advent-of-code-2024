...
class Solution:
    ...
    def check(self, x, y, score):
        if x < 0 or x >= len(self.array[0]) or y < 0 or y >= len(self.array):
            return False
        if self.array[y][x] == "#":
            return False
        return self.score_map[(x, y)] - score - 2 >= 100
        ...
        for y in range(len(self.array)):
            for x in range(len(self.array[0])):
                if self.array[y][x] in [".", "S"]:
                    total_score_saved_count += self.check(x + 2, y, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x + 1, y + 1, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x + 1, y + 1, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x, y + 2, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x - 1, y + 1, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x - 1, y + 1, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x - 2, y, self.score_map[(x, y)])
                    total_score_saved_count += self.check(x - 1, y - 1, self.score_map[(x, y)])
